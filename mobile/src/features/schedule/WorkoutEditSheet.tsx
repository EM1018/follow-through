...
export function WorkoutEditSheet({
  ...
  const [activity, setActivity] = useState<Activity | null>(workout.activity);
  ...
  const canSave = canSaveWorkout(workout, name, notes, activity);
    ...
    mutationFn: () =>
      unwrap(
        api.PATCH('/plans/{plan_id}/workouts/{workout_id}', {
          ...
          body: buildWorkoutPatch(workout, name, notes, activity),
        }),
      ),
    onSuccess: () => {
